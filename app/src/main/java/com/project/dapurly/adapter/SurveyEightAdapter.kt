...
class SurveyEightAdapter(var ctx: Context, val list: List<SurveyRowModel>) :
    RecyclerView.Adapter<SurveyEightAdapter.RowSurveyVH>() {
    private var clickListener: OnItemClickListener? = null

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RowSurveyVH {
        val view =
            LayoutInflater.from(parent.context).inflate(R.layout.row_survey_button, parent, false)
    ...
    inner class RowSurveyVH(
        ...
        val binding: RowSurveyButtonBinding = RowSurveyButtonBinding.bind(itemView)
    }
}
