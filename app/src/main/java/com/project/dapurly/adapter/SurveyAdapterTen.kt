...
import com.project.dapurly.databinding.RowImgCheckboxBinding
import com.project.dapurly.databinding.RowSurveyBinding
import com.project.dapurly.databinding.RowSurveyButtonBinding
import com.project.dapurly.model.SurveyRowModel

class SurveyAdapterTen(var ctx: Context, val list: List<SurveyRowModel>) :
    RecyclerView.Adapter<SurveyAdapterTen.RowSurveyVH>() {
    private var clickListener: OnItemClickListener? = null

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RowSurveyVH {
        val view =
            LayoutInflater.from(parent.context).inflate(R.layout.row_img_checkbox, parent, false)
    ...
    override fun onBindViewHolder(holder: RowSurveyVH, pos: Int) {
        ...
        with(holder) {
            ...
            binding.imageView.setImageDrawable( ctx.resources.getDrawable(item.image as Int))
            binding.txtName.text = item.description
    ...
    inner class RowSurveyVH(
        ...
        val binding: RowImgCheckboxBinding = RowImgCheckboxBinding.bind(itemView)
    }
}
